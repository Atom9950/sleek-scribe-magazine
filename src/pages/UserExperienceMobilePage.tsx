...
const UserExperienceMobilePage = () => {
  ...
  const articleData = {
    ...
    image: "/third.jpg",
    fullContent: `
      <p>লেখাটি রাজ্যের নৈতিক ও সামাজিক অবক্ষয়ের চিত্র তুলে ধরে, যেখানে বেকারত্ব, নারী নির্যাতন ও অবিচার বেড়েছে।</p>
...
<p>শেষে তিনি প্রশ্ন তুলেছেন, "এটাই কি সত্যিই আমার রাজ্য?", এক গভীর হতাশা ও ব্যথার সুরে।</p> `
  ...
  const handleLike = async () => {
    try {
      const newLikeCount = await toggleArticleLike(articleSlug);
      const userHasLiked = await hasUserLiked(articleSlug);
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    if (comment.trim() && !submittingComment) {
      try {
        ...
        const updatedComments = await addArticleComment(articleSlug, comment);
  ...
  const handleDeleteComment = async (commentId: string) => {
    try {
      const updatedComments = await deleteArticleComment(articleSlug, commentId);
      setComments(updatedComments);
    } catch (error) {
      ...
      <section className="max-w-4xl mx-auto px-6 py-12">
        <article className="space-y-8">
          ...
          <div className="flex items-center space-x-6 pt-8">
            ...
            <button onClick={handleShare} className="flex items-center space-x-2 text-gray-500 hover:text-black transition-colors">
